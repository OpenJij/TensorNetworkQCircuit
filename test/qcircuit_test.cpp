...
TEST(CALCULATION_TEST, GHZ_STATE) {
    using namespace std;
...
TEST(CALCULATION_TEST, LOOP_TEST) {
    ...
    //|0...111....0>
    QCircuit circuit111(topology, init_qbits, circuit.site());
    circuit111.apply(X(0), X(1), {"Cutoff", 1E-5}); // flip the qubit number (6,11)
    circuit111.apply(X(2), Id(3), {"Cutoff", 1E-5}); //flip the qubit number 10
